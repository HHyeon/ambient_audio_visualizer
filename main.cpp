...
int main()
{
   RenderWindow window(VideoMode(WIDTH,HEIGHT),"ambient_record",Style::None|Style::Titlebar);
   ...
   while(window.isOpen())
   {
      ...
      if(audiodata_queue_fft.size() > fft_one_sample_size*2)
      {
         ...
         levelbox_level.setSize(Vector2f(queuelevel*100, 20));
         ...
         {
            int sample_l = (abs(fftsample_l[i])/max*multiplier)*yheightmax;
            ...
            spectrum_data_ch_l.append(Vertex(Vector2f((barxpos*barinterval)+xpush ,ypush+bars_fadeoff_effect_ch_l[barxpos]),Color::White));
            spectrum_data_ch_l.append(Vertex(Vector2f((barxpos*barinterval)+xpush ,ypush-bars_fadeoff_effect_ch_l[barxpos]),Color::White));

            int sample_r = (abs(fftsample_r[i])/max*multiplier)*yheightmax;
            ...
            spectrum_data_ch_r.append(Vertex(Vector2f((barxpos*barinterval)+xpush ,ypush+bars_fadeoff_effect_ch_r[barxpos]),Color::Yellow));
            spectrum_data_ch_r.append(Vertex(Vector2f((barxpos*barinterval)+xpush ,ypush-bars_fadeoff_effect_ch_r[barxpos]),Color::Yellow));
            ...
            if(barxpos%100 == 0)
            {
               spectrum_data_ch_r.append(Vertex(Vector2f((barxpos*barinterval)+xpush,ypush-30),Color::Green));
               spectrum_data_ch_r.append(Vertex(Vector2f((barxpos*barinterval)+xpush,ypush+30),Color::Green));
            }
            else
            {
               if(barxpos%10 == 0)
               {
                  spectrum_data_ch_r.append(Vertex(Vector2f((barxpos*barinterval)+xpush,ypush-20),Color::Red));
                  spectrum_data_ch_r.append(Vertex(Vector2f((barxpos*barinterval)+xpush,ypush+20),Color::Red));
               }
            }
            barxpos++;
